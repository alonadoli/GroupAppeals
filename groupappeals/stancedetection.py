"""Stance detection functionality."""
...
from .pre_and_post_processing import extract_clean_stance_label, extract_group_from_hypothesis

# =============================================================================
# SHARED HELPER FUNCTIONS
# =============================================================================

def generate_stance_nli_hypotheses(group):
    """
    Create stance detection hypotheses.
    ...
    """
    return [
        f"The text is positive towards {group}.",
        f"The text is negative towards {group}.",
        f"The text is neutral, or contains no stance, towards {group}."
    ]
...
def detect_stance(texts, groups, model_name="rwillh11/mdeberta_NLI_stance_NoContext", batch_size=32, device=None):
    """
    ...
    Args:
        ...
        model_name (str): The Hugging Face model to use for stance classification
        batch_size (int): Number of examples to process at once
        device (str, optional): Device to use for computation ('cuda', 'mps', or 'cpu')
        
    Returns:
        list: List of stance labels ('positive', 'negative', 'neutral', or 'unknown' for NaN groups)
    ...
    try:
        # Check inputs
        if not texts:
            raise ValueError("No texts provided for stance detection")
        ...
        if missing_groups > 0:
            raise ValueError(
                ...
                f"Please filter out missing group references before stance detection."
            )   
        ...
        for i in range(0, len(texts), batch_size):
            ...
            for text, group in tqdm(zip(batch_texts, batch_groups), 
                                   ...
                                   desc=f"Detecting stance (batch {i//batch_size + 1})"):
                try:
                    # Create hypotheses
                    hypotheses = generate_stance_nli_hypotheses(group)
                    
                    # Get classification result
                    result = classifier(text, hypotheses, multi_label=False)
                    
                    # Add top prediction to results
                    batch_results.append({
                        'stance': result['labels'][0],
                        'confidence': result['scores'][0]
                        })
                except Exception as e:
                    print(f"Warning: Failed to process stance for text '{text[:50]}...' and group '{group}': {str(e)}")
                    batch_results.append({
                        'stance': "unknown",
                        'confidence': 0.0
                    })
        ...
        error_msg = f"Error during stance detection: {str(e)}"
        print(error_msg)
        raise RuntimeError(error_msg) from e

def process_stance_csv(input_file, text_column="text", group_column="Exact.Group.Text", 
                      output_file=None, model_name="rwillh11/mdeberta_NLI_stance_NoContext", batch_size=32,
                      device=None, clean_labels=False, quality_control=False):
    """
    Process a CSV file to detect stance towards groups.
    ...
    Args:
        ...
        model_name (str): The model to use for stance classification
        batch_size (int): Number of examples to process at once (default: 32)
        device (str, optional): Device to use for computation ('cuda', 'mps', or 'cpu')
        clean_labels (bool): Whether to extract clean stance labels ('positive', 'negative', 'neutral')
        quality_control (bool): Whether to run quality control checks on group extraction
        
    Returns:
        pd.DataFrame: DataFrame containing the original data plus a "Stance" column
    ...
    try:
        ...
        # Detect stance
        stances = detect_stance(texts, groups, model_name, batch_size, device)
        
        # Add stance to DataFrame
        df["Stance"] = [r['stance'] for r in stances]
        df["Stance_Confidence"] = [r['confidence'] for r in stances]
        
        # Apply optional postprocessing steps
        if clean_labels:
            print("Extracting clean stance labels...")
            df['Stance_Clean'] = df['Stance'].apply(extract_clean_stance_label)
        
        if quality_control:
            print("Running quality control checks on group extraction...")
            df['Group_from_stance'] = df['Stance'].apply(lambda x: extract_group_from_hypothesis(x, 'stance'))
            
            # Check for mismatches
            mismatched_stance = df[df[group_column] != df['Group_from_stance']]
            if len(mismatched_stance) > 0:
                print(f"⚠️ Warning: Found {len(mismatched_stance)} rows where extracted group from stance doesn't match {group_column}")
                print("Sample mismatches:")
                print(mismatched_stance[[group_column, 'Group_from_stance']].head())
            else:
                print("✅ Stance quality control passed: all groups match")
            
            # Remove temporary quality control column
            df = df.drop('Group_from_stance', axis=1)
        ...
        error_msg = f"Error processing CSV file for stance detection: {str(e)}"
        print(error_msg)
        raise
